...
import {
  PassThrough, pipeline as sync_pipeline,
} from 'stream'
...
'The batch Executor'.doubt(async () => {
  await 'allows multiple queries to run'
      .because(async () => {
      ...
      `
        ...
        await pipeline(
            ...
            async function* (source) {
              for await (const {
                stream,
              } of source) yield* stream
            },
            async source => {
              for await (const chunk of source) results.push(chunk)
            },
        )
  ...
  const count = 4
  await 'allows multiple subscriptions to be loadbalanced'
      .because(async () => {
        const executor = new Executor({
          schema,
          rootValue      : resolvers,
        ...
        await pipeline(
            ...
            async source => {
              try {
                for await (const {
                  id, stream,
                } of source) streams.set(id, stream)
